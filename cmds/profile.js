...
module.exports.run = async (bot, message, args) => {
    try {
        ...
        let embed = new Discord.RichEmbed()
            ...
            .setColor(`#36393f`)
            .addField(`🍬 ${msgs[1]}`, "```" +coins+"```", true)
            //.addField(`:moneybag: ${msgs[2]}`, "```"+lcoins+"```", true)
            .addField(`:triangular_flag_on_post: ${msgs[3]}`, "```"+warns+"```")
            .addField(`<:bar_chart:639517711206055957> ${msgs[4]}`, "```"+lvl+"```", true)
            .addField(`<:spy:639520314794967041> ${msgs[5]}`, "```"+`${xp}/${lvl * 5}`+"```", true)
            .addField(`:boom: ${msgs[6]}`, "```"+`${rep}`+"```")
            .addField(`🤬 ${msgs[7]}`, "```"+clan+"```", true)
            .addField(`<:couple_with_heart:639516871145816075> ${msgs[8]}`, "```"+partner+"```", true)
            .addField(`:e_mail: ${msgs[9]}`, "```"+messages+"```")
            .addField(`<:heart:639520851913342986> ${msgs[10]}`, "```"+likes+"```", true)
            .addField(`:military_medal: ${msgs[11]}`, "```"+marks+"```", true)
            .addField(`:pick: ${msgs[12]}`,"```"+bot.worklist[work].name+"```")
            //.addField(`:military_medal: ${msgs[13]}`,"```"+votes+"```")
            .setFooter(rekl, message.author.avatarURL);
        if (admin == 1) embed.addField(`:spy: Вы администратор бота`,"⁣⁣⁣⁣⁣ @<△> Natrofin#0660")
        bot.send(embed);
    } catch (err) {
        let bk = require('../botconfig.json');
